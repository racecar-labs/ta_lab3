...
x_datas = np.zeros((raw_datas.shape[0],7))
#y_datas = np.zeros((raw_datas.shape[0],3))

pose_dot = np.diff(poses, axis=0)
gt = pose_dot[:,2] > np.pi
pose_dot[gt,2] = pose_dot[gt,2] - np.pi
lt = pose_dot[:,2] < -np.pi
pose_dot[lt,2] = pose_dot[lt,2] + np.pi

# controls
#x_datas[:,3] = scipy.signal.savgol_filter( raw_datas[:,2], 11, 3, axis=0)
x_datas[:,3] = scipy.signal.savgol_filter( np.sin( raw_datas[:,2]),11,3,axis=0) 
x_datas[:,4] = scipy.signal.savgol_filter( np.cos( raw_datas[:,2]),11,3,axis=0)
x_datas[:,5:7] = raw_datas[:,3:5] # theta and controls

# Outlier filtering
#pose_dot = pose_dot[ np.abs(pose_dot[:,0]) < 0.5 ]
#pose_dot = pose_dot[ np.abs(pose_dot[:,1]) < 0.5 ]
#pose_dot = pose_dot[ np.abs(pose_dot[:,2]) < 1 ]

x_datas[0:pose_dot.shape[0]-1,0:3] = pose_dot[0:pose_dot.shape[0]-1,:]
#y_datas[0:pose_dot.shape[0]-1,:]   = pose_dot[1:pose_dot.shape[0],:]
print(x_datas.shape)

m1 = np.abs(pose_dot[:,0]) < 0.5
pose_dot = pose_dot[m1,:]
m2 = np.abs(pose_dot[:,1]) < 0.5
pose_dot = pose_dot[m2,:]
m3 = np.abs(pose_dot[:,2]) < 1
...
#y_datas = y_datas[m1,:]
#y_datas = y_datas[m2,:]
x_datas[:,0] = scipy.signal.savgol_filter( x_datas[:,0],11,3,axis=0)
x_datas[:,1] = scipy.signal.savgol_filter( x_datas[:,1],11,3,axis=0)
x_datas[:,2] = scipy.signal.savgol_filter( x_datas[:,2],11,3,axis=0)
y_datas = np.zeros((x_datas.shape[0],3))
y_datas[0:-3,:] = x_datas[3:,0:3]

print(x_datas.shape)

i = pose_dot.shape[0]-1
print(x_datas.shape)
x_datas = x_datas[0:i-3,:] # trim unused bits
y_datas = y_datas[0:i-3,:] # trim unused bits
print(x_datas.shape)
print(y_datas.shape)
